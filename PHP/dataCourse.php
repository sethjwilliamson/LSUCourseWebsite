<?php
	if (isset($_POST['getData'])) {
		...
		$response = '
          <input type="text" placeholder="Search.." id="myInputCourse" onkeyup="filterFunctionCourse()">';
		...
		while($data = $sqlSemesters->fetch_array()) {
			...
			$response .= '
				<li role="presentation">
				    <a role="menuitem" href="#" onclick="courseSelected(\''. $semester .'\', \''. $department .'\', \''. $course .'\'), onSubmit()">'. $course .'</a>
			    </li>
			';
		}
		exit($response);
