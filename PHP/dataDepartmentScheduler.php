<?php
	if (isset($_POST['getData'])) {
		...
		$response = '
          <input type="text" placeholder="Search.." id="myInputDepartment' . rand() . '" onkeyup="filterFunctionDepartment(this.parentNode)">';

		while($data = $sqlSemesters->fetch_array()) {
			$department = substr($data['department_id'], strpos($data['department_id'], '-') + 1);
			$response .= '
				<li role="presentation">
				    <a role="menuitem" href="#" onclick="getCourses(\''. $semester .'\', \''. $department .'\', this.parentNode.parentNode.parentNode.parentNode)">'. $department .'</a>
				</li>
			';
		}
		exit($response);
